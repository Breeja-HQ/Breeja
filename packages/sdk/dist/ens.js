 ...
 */
const ENSV2_UNIVERSAL_RESOLVER = "0xeeeeeeee14d718c2b47d9923deab1335e144eeee";
const DEFAULT_MAINNET_RPC_URL = "https://ethereum-rpc.publicnode.com";
let cachedClient = null;
function getEnsClient(rpcUrl) {
    if (rpcUrl) {
...
function isLikelyEnsName(value) {
 ...
 */
export async function resolveEnsName(name, rpcUrl) {
 ...
 */
export async function resolveEnsAddress(address, rpcUrl) {
    const client = getEnsClient(rpcUrl);
 ...
 */
export async function resolveRecipient(recipient, rpcUrl) {
    if (isAddress(recipient))
        return recipient;
    if (!isLikelyEnsName(recipient))
        return null;
