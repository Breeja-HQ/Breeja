...
import { createRelayerClient, type RelayerClient } from "./client.js";
import { resolveChainIdFromList } from "./chainRefs.js";
import { toSmallestUnits, toDecimalString } from "./amount.js";
import { buildTransferAuthorization, readUsdcDomainName, resolvePayerAddress } from "./signing.js";
...
const PLACEHOLDER_ADDRESS = "0x000000000000000000000000000000000000dEaD" as const;

interface RelayerRouteEntry {
  route: RouteType;
  viable: boolean;
  reason?: string;
  custody: Custody;
...
interface RelayerQuoteResponse {
  viable: boolean;
  routes: RelayerRouteEntry[];
  recommended: RelayerRouteEntry | null;
}

interface RelayerPayResponse {
  id: string;
  decision: RelayerRouteEntry;
}

type RelayerPaymentStatus = {
  id: string;
  nonce: `0x${string}`;
  fromChainId: number;
  toChainId: number;
...
interface RelayerChainsResponse {
  chains: ChainInfo[];
}

function toSdkRoute(entry: RelayerRouteEntry): Route {
  return {
    type: entry.route,
    viable: entry.viable,
    reason: entry.reason,
...
function toSdkPayment(status: RelayerPaymentStatus): Payment {
  return {
    id: status.id,
    status: status.state,
...
export class Breeja {
  ...
  async chains(): Promise<ChainInfo[]> {
    ...
    const response = await this.client.get<RelayerChainsResponse>("/chains");
    this.chainsCache = response.chains;
    return response.chains;
  }

  private async resolveChain(ref: ChainRef): Promise<number> {
   ...
   */
  private async resolveRecipientOrThrow(recipient: Recipient): Promise<Address> {
  ...
  async quote(request: QuoteRequest): Promise<Quote> {
    ...
    const response = await this.client.post<RelayerQuoteResponse>("/quote", {
      fromChainId,
      toChainId,
      payer,
      recipient,
  ...
  async pay(request: PayRequest): Promise<Payment> {
    ...
    if (!known) {
      throw new BreejaError(
        "UnsupportedChain",
        `No known SourceVault/USDC address for chain id ${fromChainId} — cannot build a signable authorization`,
        { fromChainId },
      );
    }
    ...
    const response = await this.client.post<RelayerPayResponse>("/pay", {
      fromChainId,
      toChainId,
      payer,
      recipient,
  ...
  async status(paymentId: string): Promise<Payment> {
    const status = await this.client.get<RelayerPaymentStatus>(`/status/${paymentId}`);
    ...
    const run = async (): Promise<void> => {
      ...
      try {
        ...
        for await (const frame of readSseFrames(response)) {
          if (stopped) return;
          if (frame.event !== "state" && frame.event !== "error") continue;

          const payment = toSdkPayment(JSON.parse(frame.data) as RelayerPaymentStatus);
          onUpdate(payment);
  ...
  history(_filter: HistoryFilter): Promise<Payment[]> {
    throw new Error(
      "Breeja.history() is not yet implemented: the relayer has no subgraph-backed history endpoint. " +
