...
import { z } from "zod";
import { BreejaError, type ChainRef, type RoutePreference } from "@breeja/sdk";
...
if (spendCapConfig.usingPerCallDefault) {
  console.error(
    `[breeja-mcp] BREEJA_MAX_PAYMENT_USDC not set; defaulting per-call cap to ${spendCapConfig.perCallCapUsdc} USDC.`,
  );
}
if (spendCapConfig.usingSessionDefault) {
  console.error(
    `[breeja-mcp] BREEJA_MAX_SESSION_USDC not set; defaulting per-session cap to ${spendCapConfig.sessionCapUsdc} USDC.`,
  );
}
...
const chainRefSchema = z.union([z.string(), z.number()]) as z.ZodType<ChainRef>;
const preferenceSchema = z.enum(["fast", "cheap", "trustless"]) as z.ZodType<RoutePreference>;
const addressSchema = z
  .string()
  .regex(/^0x[a-fA-F0-9]{40}$/, "must be a 0x-prefixed 20-byte address") as z.ZodType<`0x${string}`>;
...
server.registerTool(
  "breeja_quote",
  {
    ...
    description:
      "Get ranked, viable routes for a cross-chain USDC payment with fees and ETAs. Read-only — does not move funds.",
    inputSchema: {
...
server.registerTool(
  "breeja_pay",
  {
    ...
    description:
      "MOVES REAL FUNDS. Executes a live cross-chain USDC payment on Breeja — this is not a simulation or a dry run. " +
  ...
  async ({ from, to, amount, recipient, preference }) => {
    ...
    try {
      const payment = await breeja.pay({
        ...
        recipient,
        preference,
        signer: { type: "private-key", key: signerKey as `0x${string}` },
      });
...
server.registerTool(
  "breeja_history",
  {
    ...
    description:
      "List past Breeja payments for an address. Read-only. Note: history is not yet backed by a live " +
